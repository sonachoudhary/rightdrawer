...
import { getCustomerPositionAppointment } from "../../../actions/common/signin";
...
class BarberWalk extends Component {
	...
	componentDidMount() {
		...
		setInterval(() => {
    	    ...
    	    that.props.getCustomerPositionAppointment();    	
	...
	render() {
		...
		return (
			<View style={styles.container}>
				...
				<Button transparent onPress={() => { (this.props.userType == 'Teacher') ? Actions.CustomerHome() : Actions.CustomerHome() } }>
	              <Icon
	                name="md-arrow-back"
	                style={{
	                  fontSize: 24,
...
function bindActions(dispatch) {
	return {
		getCustomerPositionAppointment: appointment_id => dispatch(getCustomerPositionAppointment(appointment_id)),
	};
}
