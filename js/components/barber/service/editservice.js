...
import { updateBarberServices, delBarberServices } from "../../../actions/common/booking";
...
function mapStateToProps(state) {
  return {
    ...
    editServiceData: state.booking.editServiceData   
  };
}
...
class EditService extends Component {
  ...
  submit(values) { 
    ...
    this.props.updateBarberServices(this.props.editServiceData.id,values);
  } 
      ...
      <Container>
        <Content style={{ backgroundColor: "#0D1214"}}>
             ...
             <View style={{ flexDirection:'row', marginTop:20 }} >
              ...
              <Button
                  block
                  onPress={() => this.props.delBarberServices(this.props.editServiceData.id)}
                  style={[styles.nameBtn,{backgroundColor:"#392F2A", marginTop:50, marginLeft:30, marginRight:30 }]} >
                  <Text style={{ fontWeight: "600", color: "#FF4444" }}>
                    Delete Service
                  </Text>
              </Button>
...
function bindActions(dispatch) {
  return {
    updateBarberServices:(id,data) => dispatch(updateBarberServices(id,data)),    
    delBarberServices:(id) =>  dispatch(delBarberServices(id)),    
  };
}

EditService = reduxForm({
  form: "CreateServiceForm", // a unique name for this form
  validate
})(EditService);

EditService = connect(state => ({
  initialValues: {
    servicename: state.booking.editServiceData.name,
    serviceprice: state.booking.editServiceData.price
  }
}))(EditService);


EditService = connect(mapStateToProps, bindActions)(EditService);
