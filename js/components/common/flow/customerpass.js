...
import CustpasswordForm from "./custformpassword";

import styles from "./styles";
const deviceHeight = Dimensions.get("window").height;

class Customerpass extends Component {
  static propTypes = {
  ...
  //async function getUserInfo(){
    try {
      const retrievedItem = await AsyncStorage.getItem('customerRegistration.name');
  ...
  render() {
    return(
      <Container style={{ backgroundColor: "#fff" }}>
        ...
        >
          <Left>
            <Button transparent onPress={() => Actions.customersignup()}>
              <Icon
                name="md-arrow-back"
          ...
          <View style={{ flex: 1 }}>
            ...
            <Content style={{ backgroundColor: commonColor.brandSecondry }}>
              <CustpasswordForm isFetching={this.props.isFetching} />
            </Content>
          </View>
