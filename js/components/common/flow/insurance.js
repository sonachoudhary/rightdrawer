...
class Insurance extends Component {
  ...
  callAction(value){
    for (var i = 0; i < this.state.documentTypes.length; i++) {
      if (this.state.documentTypes[i].key === value) {
        ...
        this.forceUpdate()
        this.setState({
          selectedInsurance: this.state.documentTypes[i].key
        })
      } else {
  ...
  insuranceFormAsync = async (values) => {
    
    try {
      await AsyncStorage.setItem('driverRegistration.insurance', JSON.stringify(values));
    } catch (error) {
  ...
  licenseSubmit() {
    ...
    var values = { "insurance":this.state.selectedInsurance}
    this.insuranceFormAsync(values)
  }
  ...
  render() {
    return (
      <Container style={{ backgroundColor: "#fff" }}>
        ...
        >
          <Left>
            <Button transparent onPress={() => Actions.cpr()}>
              <Icon
                name="md-arrow-back"
        ...
        <Content style={{ backgroundColor: "#FFFFFF" }}>
          ...
          <Text style={styles.topheading}>Do you have personal liability insurance?</Text>
