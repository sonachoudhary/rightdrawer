...
class NameForm extends Component {
	...
	async componentDidMount() {
		...
		const sessionData = await AsyncStorage.getItem('driverRegistration');
    ...
    getUserInfo =  async() => {
    	    const fulldata = await AsyncStorage.getItem('driverRegistration.name');
	...
	nameFromAsync = async (values) => {
	    ...
	    try {
	      await AsyncStorage.setItem('driverRegistration.name', JSON.stringify(values));
	    } catch (error) {
	      
	    }
	    Actions.createpass({fname: 'Ashish'})
	};
