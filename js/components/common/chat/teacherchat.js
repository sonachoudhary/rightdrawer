...
import { View, Image, Platform, Dimensions, FlatList,Alert, 
  TouchableNativeFeedback as TouchableNative, TouchableOpacity, 
  ScrollView, KeyboardAvoidingView, 
  TextInput, TouchableHighlight, Keyboard ,ImageBackground,BackHandler } from "react-native";
import {
  Tab,
  Tabs, 
  ScrollableTab,
...
class Teacherchat extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      otherUserName: 'Samdeesh Verma',//this.props.contact_user.fname+" "+this.props.contact_user.lname,
  ...
  savechatApi(userId,otherUserId,message,time){
    ...
    this.props.saveChat(this.props.jwtAccessToken, data);
  }
  ...
  _retrieveid = async () => {
    var that=this;
  try {
    const otheruserId = await SyncStorage.get('otherteacherId');
    const otheruserName = await SyncStorage.get('otherteacherName');
    const otherclassName = await SyncStorage.get('otherclassName');
     // alert(otheruserId)
    this.setState({otherUserId:otheruserId})
    this.setState({otherUserName:otheruserName})
    this.setState({otherclassName:otherclassName})
...
_pickImage() {
  ...
  getchatdata(){
    ...
    fetch(`${config.serverSideUrl}/get_chathistory1/${this.props.userid}/${this.state.otherUserId}`, {
          method: "GET",
          headers: {
      ...
      <Container style={{backgroundColor:'#fff'}}>
          <ImageBackground source={ require('../../../../assets/images/Mask_Group_37.png')} style={{width: width,  resizeMode:'cover',height:90}}> 
                       <View style={{marginTop:10, flexDirection:'row'}}>
                        <Button transparent onPress={()=>Actions.Teacherchatlist()}>
                          <Icon
                            name="md-arrow-back"
                        ...
                        <View>
                          <Text style={{color:'#fff',fontSize:18,marginLeft:30,paddingTop:15}}>{this.state.otherUserName}</Text>
                          <Text style={{color:'#fff',fontSize:13,marginLeft:30,paddingTop:0}}>Class: {this.state.otherclassName}</Text>
                        </View>
                      </View>
           ...
           <Content> 
          <View style={{ backgroundColor:'#FFF',  color:'#707E85',borderTopLeftRadius:20,borderTopRightRadius:20,borderBottomWidth:1,borderColor:'lightgray',}}>
...
constructor(props) {
  ...
  render() {
    ...
    var bubbleStyles = this.props.direction === 'left' ? [styles.messageBubble, styles.messageBubbleLeft2] : [styles.messageBubble, styles.messageBubbleRight2];
      ...
      <View>
        <View style={{justifyContent: 'space-between', flexDirection: 'row',width:deviceWidth-20}}>
          ...
          <Modal
            ...
            >
            <View style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)',height:deviceHeight/1.5, }}>
...
function bindActions(dispatch) {
  return {
    ...
    saveChat:(jwtAccessToken,data) => dispatch(saveChat(jwtAccessToken,data)),
