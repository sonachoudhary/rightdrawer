  ...
  return (dispatch, getState) =>
    fetch(`${config.serverSideUrl}:${config.port}/api/syncData`, {
      ...
      .then(data => {
        if (
          data.success === true &&
          data.tripRequest != null &&
          data.trip === null
        ) {
          if (data.tripRequest.tripRequestStatus === "arrived") {
            dispatch({
              type: DRIVER_HAS_ARRIVED,
              payload: data.tripRequest
            });
          } else {
            dispatch(tripRequestSyncCompleted(data.tripRequest));
          }
        } else if (
          data.success === true &&
          data.tripRequest === null &&
          ...
          dispatch(tripSyncCompleted(data.trip));
        } else {
...
export function signinAsyncExecute(data) {
  ...
  return dispatch => {
    ...
    fetch(`${config.serverSideUrl}:${config.port}/login`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json"
